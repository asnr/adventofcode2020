...
sumGroupYeses input =
  let
    groups = T.splitOn "\n\n" input
  in
    sum $ map numCommonYesses groups

numCommonYesses group =
  let
    members = T.lines group
    choiceSets = map (Set.fromList . T.unpack) members
  in
    length $ foldl Set.intersection (head choiceSets) choiceSets
...
\nugdjbypkxashe\n\
\\n\
\nvdaqfkbyomhxptiswugrzce\n\
\xzkhfsjotigcypdnmubavweqr\n\
\ndhyrzqaekivtxgcfwosubpm\n\
